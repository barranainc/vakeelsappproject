...
export default function EditTeam2({navigation,route}) {
  const {data} = route?.params||''
  const [barDocs, setBarDocs] = useState(data?.details?.lawyer_details?.council_documents);
  const [ProfessionalDocs, setProfessionalDocs] = useState(data?.details?.lawyer_details?.professional_documents);
  const [cnicFront, setCnicFront] = useState(data?.details?.cnic_front);
  const [cnicBack, setCnicBack] = useState(data?.details?.cnic_back);
  const [profilePicture, setProfilePicture] = useState(data?.details?.picture);
  ...
  const onSubmit = async (value) => {
    ...
    try {
      setLoader(true);
      let result = await apiCall?.updateTeamMembers(obj,data?.details?._id);
  ...
  useEffect(()=>{
    ...
    setValue('cnicFront', cnicFront);
    setValue('cnicBack', cnicBack);
    setValue('professionalDocs', ProfessionalDocs);
    setValue('barCouncilDocument', barDocs);
  },[])
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: 'white'}}>
      ...
      <ImageBackground
        ...
        <AppHeader navigation={navigation} title="Edit Team" progress={'2/2'} />
        <ScrollView
