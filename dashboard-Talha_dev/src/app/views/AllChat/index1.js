...
const WhatsAppClone = () => {
  ...
  const socket = io("https://vakeelserver.barrana.io");
  const [isScrolling, setIsScrolling] = useState(false);
  const [messageSent, setMessageSent] = useState(false); // Flag to prevent duplicate message sends
  ...
  const getChatWithRoomId = async () => {
    ...
    try {
      ...
      const result = await ChatServices.getChatWithRoomId(roomId, page, 17);
      ...
      if (result.responseCode === 200) {
        ...
        if (roomId && messages?.length == 0 && selectedChat) {
          ...
          const messageData = {
            ...
            sender_id: selectedChat?.requester_id,
            receiver_id: selectedChat?.client_id,
            message: "Thank you for the request, please share more documents.",
            type: selectedChat?.type === "client_matter" ? "Matter" : "Request",
            type_name:
              selectedToggle === "client_matter"
                ? selectedChat?.client_matter_details[0]?.title
  ...
  useEffect(() => {
    if (RoomID != null || selectedChat) {
      getChatWithRoomId()
    };
  }, [selectedChat, page, RoomID]);
