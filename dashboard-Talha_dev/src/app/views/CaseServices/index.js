...
function CaseServicess() {
  ...
  const getCases = async () => {
    try {
      const result = await CaseServices.getCases();
      if (result.responseCode == 200) {
        setCases(result?.data?.categories);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const createCase = async (formData) => {
    ...
    try {
      const data = await CaseServices.createCase(obj);
  ...
  const updateCases = async (formData) => {
    const obj = {
      id: editData?._id,
      name: formData.name,
      icon: imagePreview,
    ...
    try {
      const data = await CaseServices.updateCase(obj);
      SuccessToaster(data?.message);
      setImagePreview("");
      setEditDialog(false);
      setValue2("name", "");
  ...
  const deleteCase = async () => {
    try {
      const data = await CaseServices.deleteCategory(categoryId);
      SuccessToaster(data?.message);
  ...
  return (
    <Fragment>
      ...
      >
        <Box
          component="form"
          onSubmit={handleSubmit(createCase)}
          sx={{ px: 3, py: 2 }}
        >
          <Typography
      ...
      >
        <Box
          component="form"
          onSubmit={handleSubmit2(updateCases)}
          sx={{ px: 3, py: 2 }}
        >
          <Typography
            className="heading-font"
                    ...
                    >
                      ...
                      >
                        <Box sx={{ display: "flex", justifyContent: "center" }}>
                          ...
                          <IconButton
                            onClick={() => {
                              navigate("/case-services-subcategory", { state: row });
                            }}
                          >
                            <MoreVertIcon />
                          </IconButton>
