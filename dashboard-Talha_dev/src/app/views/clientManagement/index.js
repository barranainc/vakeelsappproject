...
function UserManagement() {
  ...
  useEffect(() => {
    ...
    const delaySearch = setTimeout(() => {
      getClientData(searchText,"client", page, limit);
    }, 500); 
  ...
  const getClientData = async (searchText , userType , page, limit) => {
    setLoading(true);
  ...
  const handlePageChange = (event, value) => {
    setPage(value);
    getClientData(searchText,"client",value, limit);
  };
                ...
                {loading ? (
                  ...
                  rows.map((row) => (
                    <TableRow
                      ...
                      key={row.id}
                      onClick={() =>
                        navigate("/clients-management-details", { state: row })
                      }
                    >
                      <TableCell
