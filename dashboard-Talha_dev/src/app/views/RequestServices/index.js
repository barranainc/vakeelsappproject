...
function RequestServicess() {
  ...
  const getRequest = async () => {
    try {
      const result = await RequestServices.getRequest();
      if (result.responseCode == 200) {
        setRequest(result?.data?.categories);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const createRequest = async (formData) => {
    ...
    try {
      const data = await RequestServices.createRequest(obj);
  ...
  const updateRequest = async (formData) => {
    ...
    try {
      const data = await RequestServices.updateRequest(obj);
      SuccessToaster(data?.message);
      setImagePreview("");
      setEditDialog(false);
      setValue2("name", "");
  ...
  const deleteRequest = async () => {
    try {
      const data = await RequestServices.deleteCategory(categoryId);
      SuccessToaster(data?.message);
    ...
    <Fragment>
      ...
      >
        ...
        >
          ...
          <InputLabel sx={{ fontFamily: "Poppins" }}>Select Type</InputLabel>
          <FormControl fullWidth sx={{ mb: 2 }}>
            <Select
      ...
      >
        <Box
          component="form"
          onSubmit={handleSubmit2(updateRequest)}
          sx={{ px: 3, py: 2 }}
        >
          <Typography
            className="heading-font"
          ...
          <FormControl fullWidth sx={{ mb: 2 }}>
            <Select
              {...register2("type", { required: "Type is required" })}
              value={watch2("type") || ""}
              onChange={(e) => setValue2("type", e.target.value)}
              error={!!errors2.type}
            >
              <MenuItem value="judicial">Judicial</MenuItem>
              <MenuItem value="non_judicial">Non-Judicial</MenuItem>
            ...
            {errors2.type && (
              <Typography color="error">{errors2.type.message}</Typography>
            )}
          </FormControl>
                ...
                {request.length === 0 ? (
                  ...
                  request.map((row) => (
                    ...
                    >
                      ...
                      >
                        {row.type == "judicial" ? "Judicial" : row.type == "non_judicial" ? "Non Judicial" : ''}
                      </TableCell>
                      <TableCell
                        sx={{
                          fontSize: "15px",
                      ...
                      >
                        <Box sx={{ display: "flex", justifyContent: "center" }}>
                          ...
                          <IconButton
                            onClick={() => {
                              navigate("/request-services-subcategory", {
                                state: row,
                              });
                            }}
                          >
...
export default RequestServicess;
